...
// Cache for categories to look up names quickly
let categoryMap = {};
...
export async function initPublicShop() {
    ...
    // 1. Fetch Data (Config, Categories, Products)
    // We only fetch products with stock > 0
    ...
    // Safety Check: If store is disabled
    if (!settings.enabled) {
        app.innerHTML = `
    ...
    // create category map: { 1: 'Doors', 2: 'Windows' }
    ...
    const products = (prodRes.data || []).filter(p => {
        // Show if unlinked (null category) OR if category is visible
...
function renderShopUI(container, products, categories) {
    ...
    // Global helper for the "Enquire" button
